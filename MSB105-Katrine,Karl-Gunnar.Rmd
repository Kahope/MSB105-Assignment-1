...
ggplot(data = mtcars, 
  ...
  geom_point(data = hp_acc,
             mapping = aes(x = hp, y = acc), 
             colour = "grey80") +
  facet_wrap(~ cut_number(wt, n = 3, 
                          labels = c("Light", "Medium", "Heavy"))) +
  geom_point(aes(colour = cut_number(wt,
                                     n = 3, labels = c("Light", "Medium", "Heavy"))),
    show.legend = FALSE
  ) +
...
En løsning på denne utfordringen kan være å legge ved følgende kode i en "chunk" eller skjermbilde. 
Den vil produsere en oversikt over nødvendig programvare, for å kunne reprodusere nøyaktig samme resultater som her.

`sessionInfo()`

> *Det passer nok best å kjøre en slik kode i console-vinduet og legge ved en screenshot av infoen i appendikset, da det gjerne tar en del plass og kan virke forstyrrende inne i en artikkel (i f.eks. .pdf eller .docx filer).*
> ***I tillegg til at andre som åpner .Rmd-filen vil kun få opp deres egen sessionInfo***
